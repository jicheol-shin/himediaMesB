...
//		String sql = "select * from release_product";
	...
	public int insertReleateProduct(ReleaseProduct releaseProduct) {
		...
		try {
			...
			// util -> sql
			java.sql.Date date1 = java.sql.Date.valueOf(format.format(releaseProduct.getReleDate()));
			pstmt.setDate(2, date1);// 출하요청일자
			...
			// util -> sql
			java.sql.Date date2 = java.sql.Date.valueOf(format.format(releaseProduct.getReleDelDate()));
			pstmt.setDate(10, date2);// 납품예정일
